...
$id=$_GET["am"];
echo "$id";
$edit=mysqli_query($con,"SELECT * FROM `category` WHERE `cat_id`='$id'");
...
if($res=mysqli_fetch_assoc($data))
{
  ...
  $name=$_POST["prod"];
  //echo $name;
   mysqli_query($con,"UPDATE `category` SET `prod_id`='$ker',`cat_name`='$name' WHERE `cat_id`='$id';");
...
$data = mysqli_query($con,$query);
    ...
    <input type="text"  id="prod" name="prod" value="<?php echo $row["cat_name"];?>">
